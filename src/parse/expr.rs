...
impl<'src, 'a, 'p> PythonParser<'src, 'a, 'p> {
    ...
    fn parse_argument_declarations(&mut self, ending_token: Token<'a>) -> Result<&'a Arguments<'a>, ParseError>{
...
mod test {
    ...
    use crate::ast::constants::{ConstantPool, StringLiteral, StringStyle};
    use crate::ast::ident::SymbolTable;
    use crate::ast::tree::*;
    use crate::parse::{ExprPrec, PythonParser};
    use crate::ParseMode;
    ...
    impl<'a, 'p> TestContext<'a, 'p> {
        ...
        fn bool(&self, b: bool) -> Expr<'a> {
            let mut pool = self.pool.borrow_mut();
            let val = pool.bool(DUMMY, b).unwrap();
            self.constant(val)
        }
        fn str_lit(&self, s: &'static str) -> Expr<'a> {
            let mut pool = self.pool.borrow_mut();
        ...
        fn arg(&self, name: &'static str, type_annotation: Option<Expr<'a>>, default_value: Option<Expr<'a>>, style: ArgumentStyle) -> Arg<'a> {
            Arg {
        ...
        fn keyword_arg(&self, name: &'static str, type_annotation: Option<Expr<'a>>, default_value: Option<Expr<'a>>) -> Arg<'a> {
            self.arg(name, type_annotation, default_value, ArgumentStyle::Keyword)
        }
        fn simple_arg(&self, name: &'static str, type_annotation: Option<&'static str>) -> Arg<'a> {
            self.arg(name, type_annotation.map(|tp| self.name(tp)), None, ArgumentStyle::Positional)
        }
        fn arg_declarations(&self, args: &[Arg<'a>]) -> Arguments<'a> {
            use crate::alloc::Vec;
            let mut positional_args = Vec::new(self.arena);
            let mut keyword_args = Vec::new(self.arena);
            let mut vararg = None;
